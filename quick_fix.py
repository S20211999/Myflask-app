...
class SubOptionsPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.option_list.itemDoubleClicked.connect(self.on_option_double_clicked)
    ...
    def on_option_double_clicked(self, item):
        """Handle double click - go to input page"""
        ipc_class = item.text()
        self.option_double_clicked.emit(self.package_name, ipc_class)
...
class PadInputTab(QWidget):
    ...
    def create_existing_left_panel(self):
        ...
        self.parent_designer.add_padstack_row()
...
class PackageInputTab(QWidget):
    ...
    def create_package_input_form(self):
        """Create package input form page WITH SVG"""
        widget = QWidget()
        ...
        basic_group = QGroupBox("Package Information")
        basic_layout = QGridLayout(basic_group)
        
        basic_layout.addWidget(QLabel("Package Name:"), 0, 0)
        self.package_name_input = QLineEdit()
        basic_layout.addWidget(self.package_name_input, 0, 1)
        
        basic_layout.addWidget(QLabel("Part Number:"), 1, 0)
        self.package_part_input = QLineEdit()
        basic_layout.addWidget(self.package_part_input, 1, 1)
        ...
        self.pkg_length_input = QLineEdit("5.0")
        body_layout.addWidget(self.pkg_length_input, 0, 1)
        
        body_layout.addWidget(QLabel("Width (W) mm:"), 0, 2)
        self.pkg_width_input = QLineEdit("5.0")
        body_layout.addWidget(self.pkg_width_input, 0, 3)
        
        body_layout.addWidget(QLabel("Height (H) mm:"), 1, 0)
        self.pkg_height_input = QLineEdit("1.0")
        body_layout.addWidget(self.pkg_height_input, 1, 1)
        
        body_layout.addWidget(QLabel("Standoff mm:"), 1, 2)
        self.pkg_standoff_input = QLineEdit("0.0")
        body_layout.addWidget(self.pkg_standoff_input, 1, 3)
        
        scroll_layout.addWidget(body_group)
        
        # Lead/Pad Configuration
        lead_group = QGroupBox("Lead/Pad Configuration")
        lead_layout = QGridLayout(lead_group)
        
        lead_layout.addWidget(QLabel("Lead Count:"), 0, 0)
        self.lead_count_input = QSpinBox()
        self.lead_count_input.setRange(2, 256)
        self.lead_count_input.setValue(16)
        lead_layout.addWidget(self.lead_count_input, 0, 1)
        
        lead_layout.addWidget(QLabel("Lead Pitch mm:"), 0, 2)
        self.lead_pitch_input = QLineEdit("0.5")
        lead_layout.addWidget(self.lead_pitch_input, 0, 3)
        
        lead_layout.addWidget(QLabel("Lead Width mm:"), 1, 0)
        self.lead_width_input = QLineEdit("0.25")
        lead_layout.addWidget(self.lead_width_input, 1, 1)
        
        lead_layout.addWidget(QLabel("Lead Length mm:"), 1, 2)
        self.lead_length_input = QLineEdit("0.4")
        lead_layout.addWidget(self.lead_length_input, 1, 3)
        
        scroll_layout.addWidget(lead_group)
        
        # Thermal Pad (optional)
        thermal_group = QGroupBox("Thermal Pad (Optional)")
        thermal_layout = QGridLayout(thermal_group)
        
        self.thermal_enable = QCheckBox("Enable Thermal Pad")
        thermal_layout.addWidget(self.thermal_enable, 0, 0, 1, 2)
        
        thermal_layout.addWidget(QLabel("Thermal Width mm:"), 1, 0)
        self.thermal_width_input = QLineEdit("3.0")
        thermal_layout.addWidget(self.thermal_width_input, 1, 1)
        
        thermal_layout.addWidget(QLabel("Thermal Height mm:"), 1, 2)
        self.thermal_height_input = QLineEdit("3.0")
        thermal_layout.addWidget(self.thermal_height_input, 1, 3)
        ...
        # Generate button
        generate_btn = QPushButton("✓ Generate Package Footprint")
        ...
        """)
        generate_btn.clicked.connect(self.generate_package_footprint)
    ...
    def generate_package_footprint(self):
        ...
        package_data = {
            ...
            'thermal_enabled': self.thermal_enable.isChecked(),
            'thermal_width': float(self.thermal_width_input.text()) if self.thermal_enable.isChecked() else 0,
            'thermal_height': float(self.thermal_height_input.text()) if self.thermal_enable.isChecked() else 0
        }
...
class CourtyardLayerRow(QGroupBox):
    delete_signal = pyqtSignal()
    
    def __init__(self, row_num):
        super().__init__(f"Courtyard Layer {row_num + 1}")
    ...
    def setup_ui(self):
        ...
        self.layer_name.setPlaceholderText("e.g., Mechanical 3")
        layout.addWidget(self.layer_name, 0, 1)
        
        # Expansion
        layout.addWidget(QLabel("Expansion (mm):"), 1, 0)
        self.expansion = QLineEdit()
        self.expansion.setText("0.25")
        layout.addWidget(self.expansion, 1, 1)
        
        # Manual/Auto toggle
        layout.addWidget(QLabel("Calculation:"), 2, 0)
        ...
        calc_layout.addWidget(self.manual_toggle)
        calc_layout.addStretch()
        layout.addLayout(calc_layout, 2, 1)
        
        # Manual input area (hidden by default)
        self.manual_frame = QFrame()
        manual_layout = QGridLayout(self.manual_frame)
        
        manual_layout.addWidget(QLabel("Length (mm):"), 0, 0)
        self.manual_length = QLineEdit()
        self.manual_length.setText("0.0")
        manual_layout.addWidget(self.manual_length, 0, 1)
        
        manual_layout.addWidget(QLabel("Width (mm):"), 1, 0)
        self.manual_width = QLineEdit()
        self.manual_width.setText("0.0")
        manual_layout.addWidget(self.manual_width, 1, 1)
        
        self.manual_frame.setVisible(False)
        layout.addWidget(self.manual_frame, 3, 0, 1, 2)
    ...
    def on_manual_toggle(self):
        ...
        self.manual_frame.setVisible(self.manual_toggle.isChecked())
    
    def get_data(self):
        """Get courtyard layer data"""
        return {
            'layer_name': self.layer_name.text(),
            'expansion': self.expansion.text(),
            'manual_calculation': self.manual_toggle.isChecked(),
            'manual_length': self.manual_length.text() if self.manual_toggle.isChecked() else '',
            'manual_width': self.manual_width.text() if self.manual_toggle.isChecked() else ''
        }
...
class SettingsPanel(QDialog):
    ...
    TOOL_FIELDS = {
        'Altium': {
            'Body Settings': ['body_line_width'],
            'Courtyard Settings': ['courtyard_expansion', 'courtyard_line_width'],
            'Silkscreen Settings': ['silkscreen_airgap', 'silkscreen_line_width', 'silkscreen_follow_chamfer'],
            'Text Settings': ['text_height', 'text_width', 'text_line_width'],
            'Fiducial Settings': ['fiducial_diameter', 'fiducial_mask_opening', 'fiducial_keepout_diameter', 
                                 'fiducial_x_offset', 'fiducial_y_offset']
        },
        'Allegro': {
            'Body Settings': ['body_line_width'],
            'Courtyard Settings': ['courtyard_expansion', 'courtyard_line_width'],
            'NoProb Settings': ['noprob_top', 'noprob_bottom', 'noprob_expansion', 'noprob_line_width'],
            'DFA Bond Settings': ['dfabond_top', 'dfabond_bottom', 'dfabond_expansion'],
            'Silkscreen Settings': ['silkscreen_airgap', 'silkscreen_line_width', 'silkscreen_follow_chamfer'],
            'Text Settings': ['text_height', 'text_width', 'text_line_width'],
            'Keepout Settings': ['via_keepout_expansion', 'package_keepout_expansion', 'route_keepout_expansion']
        },
        'PADS': {
            'Body Settings': ['body_line_width'],
            'Courtyard Settings': ['courtyard_expansion', 'courtyard_line_width'],
    ...
    def create_standard_tab(self):
        """Create standard settings tab (original functionality)"""
        widget = QWidget()
    ...
    def create_right_panel(self):
        ...
        # IPC Class Settings
        ipc_group = self.create_ipc_settings_group()
        main_layout.addWidget(ipc_group)
        
        # Scrollable area for tool-specific settings
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        ...
        # Initialize with Altium fields
        self.create_tool_specific_fields('Altium')
        
        return panel
    
    def create_ipc_settings_group(self):
        """Create IPC Class settings group"""
        group = QGroupBox("IPC Class Settings")
        layout = QGridLayout(group)
        
        # Headers
        layout.addWidget(QLabel("IPC Class"), 0, 0)
        layout.addWidget(QLabel("Default"), 0, 1)
        layout.addWidget(QLabel("User Selectable"), 0, 2)
        
        # Land Pattern
        layout.addWidget(QLabel("Land Pattern"), 1, 0)
        self.land_pattern_default = QCheckBox()
        self.land_pattern_default.setChecked(True)
        layout.addWidget(self.land_pattern_default, 1, 1)
        self.land_pattern_user = QCheckBox()
        layout.addWidget(self.land_pattern_user, 1, 2)
        
        # IPC Class A
        layout.addWidget(QLabel("IPC Class A"), 2, 0)
        self.ipc_a_default = QCheckBox()
        layout.addWidget(self.ipc_a_default, 2, 1)
        self.ipc_a_user = QCheckBox()
        layout.addWidget(self.ipc_a_user, 2, 2)
        
        # IPC Class B
        layout.addWidget(QLabel("IPC Class B"), 3, 0)
        self.ipc_b_default = QCheckBox()
        self.ipc_b_default.setChecked(True)
        layout.addWidget(self.ipc_b_default, 3, 1)
        self.ipc_b_user = QCheckBox()
        self.ipc_b_user.setChecked(True)
        layout.addWidget(self.ipc_b_user, 3, 2)
        
        # IPC Class C
        layout.addWidget(QLabel("IPC Class C"), 4, 0)
        self.ipc_c_default = QCheckBox()
        layout.addWidget(self.ipc_c_default, 4, 1)
        self.ipc_c_user = QCheckBox()
        layout.addWidget(self.ipc_c_user, 4, 2)
        
        return group
    
    def create_tool_specific_fields(self, tool):
        """Create fields specific to the selected tool"""
        ...
        # Get field definitions for this tool
        tool_fields = self.TOOL_FIELDS.get(tool, {})
        
        # Create sections for each category
        for section_name, fields in tool_fields.items():
            ...
            for field in fields:
                ...
                if field in ['silkscreen_follow_chamfer']:
                    widget = QCheckBox()
                else:
                    widget = QLineEdit()
                    widget.setPlaceholderText(self.get_default_value(field))
    ...
    def get_default_value(self, field):
        ...
        defaults = {
            ...
            'fiducial_mask_opening': '2.0',
            'fiducial_keepout_diameter': '3.0',
            'fiducial_x_offset': '2.0',
            'fiducial_y_offset': '2.0',
            'via_keepout_expansion': '0.1',
            'package_keepout_expansion': '0.05',
            'route_keepout_expansion': '0.15'
        }
        return defaults.get(field, '0.0')
    
    def on_tool_changed(self, tool):
        """Handle tool selection change"""
        self.create_tool_specific_fields(tool)
    ...
    def load_from_local(self):
        ...
        for standard in standards:
            item = QListWidgetItem(standard['name'])
            item.setData(Qt.ItemDataRole.UserRole, standard)
            self.standards_list.addItem(item)
        
        self.status_label.setText(f"Loaded {len(standards)} standards from local")
        print(f"Loaded {len(standards)} standards from local storage")
    
    def save_to_local(self, standard):
        """Save a single standard to local JSON"""
        success = LocalStandardsManager.add_or_update_standard(standard)
        if success:
            self.load_from_local()  # Refresh list
            return True
        return False
    ...
    def update_permissions(self):
        ...
        # Disable IPC checkboxes
        for checkbox in [self.land_pattern_default, self.land_pattern_user,
                        self.ipc_a_default, self.ipc_a_user,
                        self.ipc_b_default, self.ipc_b_user,
                        self.ipc_c_default, self.ipc_c_user]:
    ...
    def create_new_standard(self):
        ...
        self.standards_list.clearSelection()
        self.tool_combo.setCurrentIndex(0)
        
        # Reset IPC settings
        self.land_pattern_default.setChecked(True)
        self.land_pattern_user.setChecked(False)
        self.ipc_a_default.setChecked(False)
        self.ipc_a_user.setChecked(False)
        self.ipc_b_default.setChecked(True)
        self.ipc_b_user.setChecked(True)
        self.ipc_c_default.setChecked(False)
        self.ipc_c_user.setChecked(False)
        ...
        self.status_label.setText("New standard created")
        QMessageBox.information(self, 'New Standard',
                              'Configure settings and click "Save Standard"')
    
    def fetch_from_server(self):
        """Fetch standards from server and save to local JSON"""
        try:
            self.status_label.setText("Fetching from server...")
            
            # Use parent's server connection if available
            if self.parent_server:
                success, result = self.parent_server.get_standards()
            else:
                response = requests.get(f'{self.server_url}/standards', timeout=5)
            ...
            if success:
                ...
                # Refresh list from local
                self.load_from_local()
                
                self.status_label.setText(f"Downloaded {len(result)} standards from server")
                QMessageBox.information(self, 'Success', 
                                      f'Successfully downloaded and saved {len(result)} standards')
            else:
                self.status_label.setText("Failed to load")
                QMessageBox.warning(self, 'Error', f'Failed: {result}')
    ...
    def save_standard(self):
        """Save standard to local JSON and upload to server (admin/manager only)"""
        print(f"save_standard called")
        print(f"token: {bool(self.token)}")
        print(f"is_admin_manager: {self.is_admin_manager}")
        
        if not self.is_admin_manager:
            QMessageBox.warning(self, "Permission Denied",
                              f"Only admin/manager can save standards. Your role: {str(self.user_role)}")
            return
        
        standard_name = self.standard_name_input.text().strip()
        if not standard_name:
            QMessageBox.warning(self, "Error", "Enter standard name")
            return
        ...
        config = {
            ...
            # IPC settings
            'land_pattern_default': self.land_pattern_default.isChecked(),
            'land_pattern_user': self.land_pattern_user.isChecked(),
            'ipc_a_default': self.ipc_a_default.isChecked(),
            'ipc_a_user': self.ipc_a_user.isChecked(),
            'ipc_b_default': self.ipc_b_default.isChecked(),
            'ipc_b_user': self.ipc_b_user.isChecked(),
            'ipc_c_default': self.ipc_c_default.isChecked(),
            'ipc_c_user': self.ipc_c_user.isChecked(),
        }
        ...
        try:
            ...
            if self.save_to_local(standard):
                ...
                # 2. Upload to server (if connected)
                if self.parent_server:
                    print(f"Uploading standard '{standard_name}' to server...")
                    ...
                    response = self.parent_server.session.post(
                        f'{self.parent_server.api_url}/standards/save',
                        json=payload,
                        timeout=5
                    )
                    
                    if response.status_code in [200, 201]:
                        self.status_label.setText("Saved locally + uploaded to server!")
                        QMessageBox.information(self, 'Success',
                                              f'Standard "{standard_name}" saved locally and uploaded to server!')
                    else:
                        error_msg = response.json().get('error', 'Upload failed')
                        self.status_label.setText("Saved locally (upload failed)")
                        QMessageBox.warning(self, 'Partial Success',
                                          f'Saved locally but upload failed: {error_msg}')
                else:
                    QMessageBox.information(self, 'Success',
    ...
    def load_settings_from_config(self, config):
        ...
        try:
            # Load basic settings
            self.standard_name_input.setText(config.get('standard_name', ''))
            
            tool = config.get('tool', 'Altium')
            self.tool_combo.setCurrentText(tool)
            
            # Load IPC settings
            self.land_pattern_default.setChecked(config.get('land_pattern_default', True))
            self.land_pattern_user.setChecked(config.get('land_pattern_user', False))
            self.ipc_a_default.setChecked(config.get('ipc_a_default', False))
            self.ipc_a_user.setChecked(config.get('ipc_a_user', False))
            self.ipc_b_default.setChecked(config.get('ipc_b_default', True))
            self.ipc_b_user.setChecked(config.get('ipc_b_user', True))
            self.ipc_c_default.setChecked(config.get('ipc_c_default', False))
            self.ipc_c_user.setChecked(config.get('ipc_c_user', False))
    ...
    # Add this helper method to get_settings to prevent widget deletion errors
    def get_settings(self):
        """Get all current settings - with safe widget access"""
        settings = {}
        
        # Safely access field widgets
        for field, widget in self.field_widgets.items():
            try:
                if isinstance(widget, QCheckBox):
                ...
                # Widget was deleted, use default
                settings[field] = self.get_default_value(field)
    ...
    def set_settings(self, settings):
        ...
        try:
            # Set tool first (this will recreate field widgets)
            ...
            # Set IPC settings
            self.land_pattern_default.setChecked(settings.get('land_pattern_default', True))
            self.land_pattern_user.setChecked(settings.get('land_pattern_user', False))
            self.ipc_a_default.setChecked(settings.get('ipc_a_default', False))
            self.ipc_a_user.setChecked(settings.get('ipc_a_user', False))
            self.ipc_b_default.setChecked(settings.get('ipc_b_default', True))
            self.ipc_b_user.setChecked(settings.get('ipc_b_user', True))
            self.ipc_c_default.setChecked(settings.get('ipc_c_default', False))
            self.ipc_c_user.setChecked(settings.get('ipc_c_user', False))
...
class FootprintDesigner(QMainWindow):
    ...
    def load_standards(self):
        """Load standards from server into dropdown"""
        if not self.token:
            print("Cannot load standards: no token")
            return
        
        print("Loading standards from server")
        success, result = self.server.get_standards()
        ...
        if success:
            ...
            print(f"Loaded {len(result)} standards")
        else:
    ...
    def adjust_pads_for_origin_change(self, old_origin_x, old_origin_y, new_origin_x, new_origin_y):
        ...
        print(f"Adjusting pads: delta_x={delta_x}, delta_y={delta_y}")  # Debug
        ...
        for row in self.padstack_rows:
            ...
            if offset_from == 'origin':
                ...
                print(f"Pad: old=({current_x},{current_y}) -> new=({new_x},{new_y})")  # Debug
    ...
    # ADD THIS METHOD (or modify existing on_origin_manual_change)
    def on_origin_offset_changed(self):
        """Called when origin offset inputs change"""
        try:
            # Get old values - use current text if _last values not set
            old_x = getattr(self, '_last_origin_x', None)
            old_y = getattr(self, '_last_origin_y', None)
            
            # If this is the first call, initialize from current input values
            if old_x is None:
                old_x = Decimal(str(self.origin_offset_x_input.text() or '0'))
                self._last_origin_x = old_x
            if old_y is None:
                old_y = Decimal(str(self.origin_offset_y_input.text() or '0'))
                self._last_origin_y = old_y
                return  # Don't adjust on first initialization
            
            new_x = Decimal(str(self.origin_offset_x_input.text() or '0'))
            new_y = Decimal(str(self.origin_offset_y_input.text() or '0'))
...
############################################################################### Altium script ###############################################################################################################
    ...
    def generate_altium_script(self):
        ...
        if body_shape == 'rectangle':
            ...
            lines.append(f"Arc (Width {body_line_width*SCALE}) (Location {(center_x*SCALE)}, {(center_y*SCALE)}) (Radius {(radius*SCALE)}) (StartAngle 0) (EndAngle 360) (Layer Mechanical13)")
        ...
        if body_shape == 'round' and body_length > 0 and body_width > 0:
            ...
            # Draw circular courtyard
            lines.append(f"Arc (Width {courtyard_line_width*SCALE}) (Location {center_x*SCALE}, {center_y*SCALE}) (Radius {courtyard_radius*SCALE}) (StartAngle 0) (EndAngle 360) (Layer Mechanical15)")
