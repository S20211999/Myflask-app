"""
Activity Tracker Server - Enhanced Version
Centralized server with holiday configuration and weekend override management
UPDATES:
- Holiday configuration table and endpoints
- Weekend working day override table and endpoints
- Support for managers/admins to override weekend off days
"""
...
class ActivityTrackerServer:
    ...
    def init_database(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS activities (
                ...
                FOREIGN KEY (default_activity_id) REFERENCES default_activities(id)
            )
        ''')
        
        # Default activities table
        cursor.execute('''
        ...
        # Weekend working day overrides table (NEW)
        cursor.execute('''
    ...
    def authenticate_user(self, username, password):
        ...
        cursor.execute('''
            SELECT id, username, full_name, role, manager_id 
    ...
    def get_all_users(self):
        ...
        cursor.execute('''
            SELECT u.id, u.username, u.full_name, u.role, u.manager_id, m.full_name as manager_name
            FROM users u
        ...
        return [{'id': u[0], 'username': u[1], 'full_name': u[2], 'role': u[3], 
                 'manager_id': u[4], 'manager_name': u[5]} for u in users]
    ...
    def get_user_hierarchy(self, user_id):
        ...
        return [{'id': u[0], 'username': u[1], 'full_name': u[2], 'role': u[3]} for u in users]
    ...
    def add_user(self, data):
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (data['username'], hashed_pass, data['full_name'], 
                  data['role'], data.get('manager_id')))
    ...
    def delete_user(self, user_id):
        ...
        try:
            ...
            cursor.execute('DELETE FROM activities WHERE assigned_to = ? OR created_by = ?', 
                         (user_id, user_id))
    ...
    def update_hierarchy(self, employee_id, manager_id):
        """Update reporting relationship"""
        conn = sqlite3.connect('activity_tracker.db')
        cursor = conn.cursor()
        
        try:
            cursor.execute('UPDATE users SET manager_id = ? WHERE id = ?', 
    ...
    def add_activity(self, data):
        ...
        cursor.execute('''
            INSERT INTO activities (title, description, frequency, time, 
                                   created_by, assigned_to, due_date)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (data['title'], data.get('description', ''), data['frequency'],
              data['time'], data['created_by'], data['assigned_to'], 
              data.get('due_date')))
    ...
    def generate_daily_activities_for_user(self, user_id, date):
        """Generate daily activities from defaults for a specific user and date"""
        conn = sqlite3.connect('activity_tracker.db')
        cursor = conn.cursor()
        
        date_str = date if isinstance(date, str) else date.strftime('%Y-%m-%d')
        
        # Get all default daily activities
        cursor.execute('SELECT id, title, frequency, time FROM default_activities WHERE frequency = "daily"')
        defaults = cursor.fetchall()
        
        for default in defaults:
            default_id, title, frequency, time = default
            
            # Check if activity already exists for this user and date
            cursor.execute('''
                SELECT id FROM activities 
                WHERE assigned_to = ? AND title = ? AND due_date = ?
            ''', (user_id, title, date_str))
            
            if not cursor.fetchone():
                # Create the activity with link to default
                cursor.execute('''
                    INSERT INTO activities (title, description, frequency, time, 
                                        created_by, assigned_to, due_date, status, default_activity_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?)
                ''', (title, '', frequency, time, 1, user_id, date_str, default_id))
        
        conn.commit()
        conn.close()
    
    def generate_daily_activities_for_month(self, user_id, year, month):
        """Generate daily activities for entire month"""
        ...
        while current_date < last_day:
            self.generate_daily_activities_for_user(user_id, current_date)
            current_date += timedelta(days=1)
    
    def get_activities(self, user_id, date=None):
        """Get activities for a user on a specific date"""
        if date is None:
            date = datetime.now().strftime('%Y-%m-%d')
        
        self.generate_daily_activities_for_user(user_id, date)
        
        conn = sqlite3.connect('activity_tracker.db')
        cursor = conn.cursor()
        
        cursor.execute('''
            SELECT a.id, a.title, a.description, a.frequency, a.time,
                   a.status, a.due_date, u.full_name as assigner_name
            FROM activities a
            LEFT JOIN users u ON a.created_by = u.id
            WHERE a.assigned_to = ? AND a.due_date = ?
            ORDER BY a.time
        ''', (user_id, date))
        ...
        return [{
            ...
            'frequency': a[3], 'time': a[4], 'status': a[5],
            'due_date': a[6], 'assigner_name': a[7]
        } for a in activities]
    
    def get_month_activities(self, user_id, year, month):
        """Get all activities for a specific month, auto-generating daily activities"""
        self.generate_daily_activities_for_month(user_id, year, month)
        ...
        cursor.execute('''
            SELECT a.id, a.title, a.description, a.frequency, a.time,
                   a.status, a.due_date, a.completed_at, u.full_name as assigner_name,
                   a.assigned_to
            FROM activities a
            LEFT JOIN users u ON a.created_by = u.id
            WHERE a.assigned_to = ? AND a.due_date >= ? AND a.due_date < ?
            ORDER BY a.due_date, a.time
        ...
        return [{
            'id': a[0], 'title': a[1], 'description': a[2],
            'frequency': a[3], 'time': a[4], 'status': a[5],
            'due_date': a[6], 'completed_at': a[7], 'assigner_name': a[8],
            'assigned_to': a[9]
        } for a in activities]
    
    def update_activity_status(self, activity_id, status, user_role=None):
        """Update activity status with time validation"""
        conn = sqlite3.connect('activity_tracker.db')
        cursor = conn.cursor()
        
        cursor.execute('''
            SELECT time, due_date, status FROM activities WHERE id = ?
        ''', (activity_id,))
        ...
        activity_time, due_date, current_status = activity
        
        if status == 'complete' and user_role == 'employee':
            ...
            try:
                ...
                if now > activity_datetime:
                    conn.close()
                    return {'success': False, 'error': 'Cannot mark complete after assigned time'}
            except ValueError:
                pass
        ...
        cursor.execute('''
            ...
            WHERE id = ?
        ''', (status, completed_at, activity_id))
    ...
    def add_default_activity(self, data):
        """Add default daily activity"""
        conn = sqlite3.connect('activity_tracker.db')
        cursor = conn.cursor()
        
        cursor.execute('''
            INSERT INTO default_activities (title, frequency, time, created_by)
            VALUES (?, ?, ?, ?)
        ''', (data['title'], data['frequency'], data['time'], data['created_by']))
    ...
    def get_default_activities(self):
        """Get all default activities"""
        conn = sqlite3.connect('activity_tracker.db')
        cursor = conn.cursor()
        
        cursor.execute('SELECT id, title, frequency, time FROM default_activities')
        activities = cursor.fetchall()
        conn.close()
        
        return [{'id': a[0], 'title': a[1], 'frequency': a[2], 'time': a[3]} 
                for a in activities]
    
    def delete_default_activity(self, activity_id, delete_generated=False):
        """Delete default activity and optionally delete all linked generated activities"""
        conn = sqlite3.connect('activity_tracker.db')
        cursor = conn.cursor()
        
        try:
            # Delete the default activity template
            cursor.execute('DELETE FROM default_activities WHERE id = ?', (activity_id,))
            
            # If requested, delete all generated activities linked to this default
            if delete_generated:
                cursor.execute('''
                    DELETE FROM activities 
                    WHERE default_activity_id = ? AND status = 'pending'
                ''', (activity_id,))
                ...
                return {
                    'success': True, 
                    'message': f'Deleted default template and {deleted_count} pending activities'
                }
            else:
                # Just nullify the link, don't delete activities
                cursor.execute('''
                    UPDATE activities 
                    SET default_activity_id = NULL 
                    WHERE default_activity_id = ?
                ''', (activity_id,))
                
                conn.commit()
                conn.close()
                return {'success': True, 'message': 'Deleted default template only'}
    ...
    def get_holidays(self):
        ...
        cursor.execute('SELECT id, name, date FROM holidays ORDER BY date')
        holidays = cursor.fetchall()
        conn.close()
        
        return [{'id': h[0], 'name': h[1], 'date': h[2]} for h in holidays]
    ...
    # ==================== WEEKEND OVERRIDE MANAGEMENT ====================
    
    def set_working_day_override(self, date, is_working, user_id):
        """Set or update weekend working day override"""
        conn = sqlite3.connect('activity_tracker.db')
        cursor = conn.cursor()
        
        try:
            # Check if override exists
            cursor.execute('SELECT id FROM weekend_overrides WHERE date = ?', (date,))
            existing = cursor.fetchone()
            
            if existing:
                # Update existing
                cursor.execute('''
                    UPDATE weekend_overrides 
                    SET is_working = ?, modified_by = ?, modified_at = CURRENT_TIMESTAMP
                    WHERE date = ?
                ''', (is_working, user_id, date))
            else:
                # Insert new
                cursor.execute('''
                    INSERT INTO weekend_overrides (date, is_working, modified_by)
                    VALUES (?, ?, ?)
                ''', (date, is_working, user_id))
    ...
    def get_working_day_overrides(self, year, month):
        """Get weekend overrides for a specific month"""
        conn = sqlite3.connect('activity_tracker.db')
        cursor = conn.cursor()
        ...
        cursor.execute('''
            SELECT id, date, is_working, modified_by 
        ...
        return [{'id': o[0], 'date': o[1], 'is_working': bool(o[2]), 'modified_by': o[3]} 
                for o in overrides]
    
    def export_data(self, user_id, year=None, month=None):
        """Export user activities to dict for Excel"""
        conn = sqlite3.connect('activity_tracker.db')
        cursor = conn.cursor()
        ...
        if year and month:
            ...
            cursor.execute('''
                SELECT a.title, a.description, a.frequency, a.time, a.status,
                       a.due_date, a.completed_at, u.full_name as assigner
                FROM activities a
                LEFT JOIN users u ON a.created_by = u.id
                WHERE a.assigned_to = ? AND a.due_date >= ? AND a.due_date < ?
                ORDER BY a.due_date, a.time
            ...
            cursor.execute('''
                ...
                LEFT JOIN users u ON a.created_by = u.id
                WHERE a.assigned_to = ?
                ORDER BY a.due_date DESC, a.time
            ''', (user_id,))
        ...
        return [{
            'Title': a[0], 'Description': a[1], 'Frequency': a[2],
            'Time': a[3], 'Status': a[4], 'Due Date': a[5],
            'Completed At': a[6], 'Assigned By': a[7]
        } for a in activities]
            ...
            while True:
                ...
                if action == 'login':
                    ...
                    if user:
                        ...
                        self.clients[user['id']] = client_socket
                    else:
                    ...
                    response = self.add_user(request['data'])
                
                elif action == 'delete_user':
                    response = self.delete_user(request['user_id'])
                
                elif action == 'update_hierarchy':
                    response = self.update_hierarchy(request['employee_id'], 
                                                    request['manager_id'])
                
                elif action == 'get_hierarchy':
                    users = self.get_user_hierarchy(request['user_id'])
                    response = {'success': True, 'users': users}
                
                elif action == 'add_activity':
                    response = self.add_activity(request['data'])
                
                elif action == 'get_activities':
                    activities = self.get_activities(request['user_id'], 
                                                    request.get('date'))
                    response = {'success': True, 'activities': activities}
                
                elif action == 'get_month_activities':
                    activities = self.get_month_activities(request['user_id'],
                                                          request['year'],
                                                          request['month'])
                    response = {'success': True, 'activities': activities}
                
                elif action == 'update_activity':
                    response = self.update_activity_status(request['activity_id'], 
                                                          request['status'],
                                                          request.get('user_role'))
                
                elif action == 'add_default_activity':
                    response = self.add_default_activity(request['data'])
                
                elif action == 'get_default_activities':
                    activities = self.get_default_activities()
                    response = {'success': True, 'activities': activities}
                ...
                # Holiday endpoints
                    ...
                    response = self.add_holiday(request['data'])
                    ...
                    response = self.delete_holiday(request['holiday_id'])
                
                # Weekend override endpoints
                elif action == 'set_working_day_override':
                    response = self.set_working_day_override(
                        request['date'], 
                        request['is_working'],
                        request.get('user_id', 1)
                    )
                
                elif action == 'get_working_day_overrides':
                    overrides = self.get_working_day_overrides(
                        request['year'],
                        request['month']
                    )
                    response = {'success': True, 'overrides': overrides}
                
                elif action == 'export_data':
                    data = self.export_data(request['user_id'],
                                          request.get('year'),
                                          request.get('month'))
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("Activity Tracker Server - Enhanced Version")
    print("=" * 60)
    ...
    print("-" * 60)
    print("New Features:")
    print("  ✓ Holiday configuration (yellow in calendar)")
    print("  ✓ Weekend off days (orange in calendar)")
    print("  ✓ Manager/Admin can override weekends as working days")
    print("  ✓ Subordinate dropdown in main window")
    print("  ✓ Toggle switches for activity completion")
    print("=" * 60)
    server.start()
...
class AdminPanel(QDialog):
    ...
    def delete_default(self, activity_id):
        """Delete default activity and optionally remove generated activities"""
        # Ask user if they want to delete generated activities too
        reply = QMessageBox.question(
            self, 
            'Delete Default Activity', 
            'Do you want to delete this default activity?\n\n'
            'Yes - Delete template only (keeps existing activities)\n'
            'No - Delete template AND all generated activities from this template',
        ...
        delete_generated = (reply == QMessageBox.StandardButton.No)
        ...
        if conn.connect():
            ...
            if response.get('success'):
                self.load_default_activities()
                if delete_generated:
                    QMessageBox.information(self, 'Success', 'Default activity and all generated activities deleted')
                else:
...
class AlwaysOnWizard(QWidget):
    """Always-on display wizard - semi-transparent, draggable, non-interactive wallpaper-like widget"""
    def __init__(self, user_id, user_data, parent=None):
        ...
        # Window flags for always-on behavior
        self.setWindowFlags(
            Qt.WindowType.FramelessWindowHint | 
            Qt.WindowType.WindowStaysOnBottomHint |  # Stay behind other windows
            Qt.WindowType.Tool |  # Don't show in taskbar
            Qt.WindowType.WindowTransparentForInput  # Click-through disabled for dragging
        )
        
        # Remove click-through to allow dragging
        self.setWindowFlags(
            Qt.WindowType.FramelessWindowHint | 
        ...
        self.refresh_timer.start(1800)
        
        # Position at bottom-right corner
        self.position_widget()
    
    def position_widget(self):
        """Position widget at bottom-right corner of screen"""
    ...
    def init_ui(self):
        ...
        title = QLabel('🧙 Today\'s Activities')
        ...
        self.date_label.setStyleSheet('font-size: 12px; color: rgba(255, 255, 255, 200);')
    ...
    def load_today_activities(self):
        ...
        for row, act in enumerate(activities):
            ...
            if act['status'] == 'complete':
                ...
                title_item.setForeground(QColor(200, 200, 200))
...
class SystemTray:
    ...
    def __init__(self, app, main_window):
        ...
        self.tray_icon = QSystemTrayIcon(self.create_icon(), app)
        self.menu = QMenu()
    ...
    def create_icon(self):
        ...
        gradient = QColor('#667eea')
        painter.setBrush(QBrush(gradient))
        painter.setPen(Qt.PenStyle.NoPen)
        painter.drawRoundedRect(16, 16, 96, 96, 24, 24)
        
        painter.setPen(QPen(QColor('white'), 2))
        painter.setBrush(Qt.BrushStyle.NoBrush)
        painter.drawRoundedRect(32, 40, 64, 56, 8, 8)
        
        painter.setPen(QPen(QColor('white'), 3))
        painter.drawLine(48, 32, 48, 44)
        painter.drawLine(80, 32, 80, 44)
