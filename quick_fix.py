...
class PublicGoogleSheetsManager:
    ...
    No authentication required - uses public share link
    """
    ...
    def _get_sheet_id_from_link(self, link):
    ...
    def _connect_to_sheet(self):
        """Connect to publicly shared Google Sheet"""
        try:
            print("🔗 Connecting to public Google Sheet...")
            
            # Extract Sheet ID from link
            sheet_id = self._get_sheet_id_from_link(self.sheet_link)
            
            if not sheet_id:
                print("❌ Invalid Google Sheet link format")
                return
            
            # For publicly shared sheets, use Anonymous authentication
            self.gc = gspread.Client(auth=None)
            
            # Open the sheet by ID (works for "Anyone with link" sheets)
            ...
            print("❌ Sheet not found. Make sure:")
            print("   1. Sheet ID is correct")
            print("   2. Sheet is shared as 'Anyone with the link can edit'")
            self.spreadsheet = None
        except Exception as e:
    ...
    def save_result(self, username, name, group, score, total, timestamp):
        ...
        try:
            ...
            print(f"✓ Result saved to Google Sheet: {username} - {score}/{total} ({percentage:.2f}%)")
            return True
...
def main():
    ...
    print("="*70)
    print("📚 Online Exam System - Public Google Sheets")
    print("="*70)
