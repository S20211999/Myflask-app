...
# ============================================
# CONFIGURATION - GOOGLE SHEETS PUBLIC LINK
# ============================================
# Paste your Google Sheet shareable link here
GOOGLE_SHEET_LINK = "https://docs.google.com/spreadsheets/d/YOUR_SHEET_ID/edit?usp=sharing"
# ============================================

class PublicGoogleSheetsManager:
    """
    Manager for publicly shared Google Sheets (Anyone can edit)
    Uses requests library to fetch data without authentication
    """
    ...
    def _extract_sheet_id(self):
        ...
        try:
            ...
            self.sheet_id = self.sheet_link.split('/d/')[1].split('/')[0]
            print(f"✓ Sheet ID extracted: {self.sheet_id}")
        except Exception as e:
            print(f"❌ Error extracting Sheet ID: {e}")
            self.sheet_id = None
    
    def _load_sheets_data(self):
        """Load all sheet data using public CSV export URL"""
        try:
            if not self.sheet_id:
                print("❌ No valid Sheet ID")
                return
            
            print("📥 Loading data from Google Sheets...")
            
            sheet_names = ['Students', 'Schedule', 'Questions', 'Results']
            
            for sheet_name in sheet_names:
                try:
                    # Use gid=0 for first sheet, you can map sheet names to gids
                    gid = self._get_sheet_gid(sheet_name)
                    csv_url = f"https://docs.google.com/spreadsheets/d/{self.sheet_id}/gviz/query?tqx=out:csv&gid={gid}"
                    ...
                    if response.status_code == 200:
                        ...
                        self.data[sheet_name] = [line.split(',') for line in lines]
                        print(f"✓ Loaded {len(self.data[sheet_name])} rows from '{sheet_name}'")
                    else:
                        print(f"⚠ Failed to load '{sheet_name}': Status {response.status_code}")
    ...
    def _get_sheet_gid(self, sheet_name):
        """Map sheet name to gid (default 0 for first sheet)"""
        sheet_gid_map = {
            'Students': '0',
            'Schedule': '1',
            'Questions': '2',
        ...
        return sheet_gid_map.get(sheet_name, '0')
        ...
        try:
            ...
            for row in students_data[1:]:
                if len(row) >= 6:
                    ...
                    if row[0].strip() == username and row[1].strip() == password and row[5].strip() == 'Active':
                        return {
        ...
        try:
            ...
            for row in schedule_data[1:]:
                if len(row) >= 7:
                    ...
                    config_id = row[0].strip()
                    exam_group = self._get_group_from_config(config_id)
                    
                    if exam_group == group and row[6].strip() == 'Scheduled':
                        exam_date_str = row[1].strip()
                        start_time_str = row[2].strip()
                        end_time_str = row[3].strip()
                        duration = row[4].strip() if len(row) > 4 else '60'
        ...
        try:
            ...
            for row in questions_data[1:]:
                if len(row) >= 9:
                    ...
                    if row[1].strip() == group and row[8].strip() == 'Active':
                        correct_answer = str(row[7].strip()).upper()
    ...
    def save_result(self, username, name, group, score, total, timestamp):
        """Save exam result by appending to Results sheet"""
        try:
            # For now, we'll just print the result
            # To actually save to Google Sheets, you'd need write access via API
            percentage = (score / total * 100) if total > 0 else 0
            status = 'Pass' if percentage >= 60 else 'Fail'
            
            print(f"\n{'='*60}")
            print(f"📊 EXAM RESULT - Saving to Google Sheets")
            print(f"{'='*60}")
    ...
    def get_exam_duration(self, group):
        ...
        try:
            ...
            for row in schedule_data[1:]:
                if len(row) >= 5:
                    exam_group = self._get_group_from_config(row[0].strip())
                    if exam_group == group:
                        try:
...
class LoginWindow(QWidget):
    ...
    def init_ui(self):
        ...
        # Connection Status
        if self.sheets_manager.sheet_id:
            status_text = f'✓ Connected: {self.sheets_manager.sheet_id[:20]}...'
            status_color = '#4CAF50'
        else:
...
<body style="font-family: Arial;">
    ...
    <p style="color: {'green' if success else 'red'};">
        {'✓ Results saved successfully!' if success else '⚠ Results displayed (Google Sheets write requires API)'}
    </p>
...
def main():
    ...
    print("="*70)
    print("📚 Online Exam System - Public Google Sheets (Fixed)")
    print("="*70)
    
    # Initialize Public Google Sheets Manager
    sheets_manager = PublicGoogleSheetsManager(GOOGLE_SHEET_LINK)
    
    if sheets_manager.sheet_id is None:
        QMessageBox.critical(None, 'Connection Error', 
                           '❌ Failed to connect to Google Sheet.\n\n'
                           'Please verify:\n'
                           '1. Google Sheet link is correct\n'
                           '2. Sheet is shared as "Anyone with the link can edit"\n'
                           '3. Internet connection is active\n\n'
                           'Update GOOGLE_SHEET_LINK in the code')
        sys.exit(1)
