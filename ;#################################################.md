...
Replace broken logical operators:

```lisp
; Change from:
(if  txt_lyr == assytop_layer ||  txt_lyr == assybot_layer then
...
### 4. Fix Function Parameter Lists

Ensure proper SKILL function syntax:

```lisp
; Fix parameter lists like:
axlSetFindFilter ('?enabled   (list "noall"  "text")
                  '?onButtons (list "text"))
...
1. **Test Form Creation**: Ensure the form file is created properly in the correct directory
2. **Validate Form Handle**: Always check if form creation succeeded before using the form
3. **Fix All Conditionals**: Review every `if` statement for proper SKILL syntax
4. **Test Incrementally**: Load and test individual functions to isolate remaining issues

The corrected code I generated addresses these syntax issues systematically. The primary solution is adding form validation immediately after `axlFormCreate` to prevent the `nil` form handle from being passed to `axlFormSetField`. This error pattern is well-documented in the Cadence community forums and occurs frequently when form creation fails silently.[^1_1]
